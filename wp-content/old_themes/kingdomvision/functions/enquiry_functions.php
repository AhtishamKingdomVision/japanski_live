...
function post_to_third_party( $entry, $form ) {
    ...
    // pre( var_dump( is_user_logged_in() ) );
    // pre( var_dump( get_current_user_id() ) );
    // pre( var_dump( $is_stay_user ) );
    // $is_stay_user = false;
    // $is_stay_user = !is_user_logged_in() || get_current_user_id() != 14 ? true : false;
    $api_url = $is_stay_user ? 'https://stay.japanskiexperience.com/api/v1/third-party-enquiry' : 'https://trip.japanskiexperience.com/api/v1/third-party-enquiry';
    // pre( $api_url );
    $api_url .= $query_param;
    $endpoint_urls = [
        $api_url
    ...
    foreach ( $endpoint_urls as $url ) {
        ...
        if ( is_wp_error( $response ) ) {
            cf_log( 'API Error for ' . $url . ': ' . $response->get_error_message(), 'error.txt', false, true );
            continue;
        }
...
function get_cached_customers() {
    ...
    $api_url = $is_stay_user ? 'https://stay.japanskiexperience.com/api/v1/customers' : 'https://trip.japanskiexperience.com/api/v1/customers';
