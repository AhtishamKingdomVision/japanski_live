...
const BookingManager = (function() {
    ...
    const RoomRenderer = {
        ...
        renderBookingSummaryCard: function(cart) {
            ...
            items.forEach((it) => {
                ...
                if (deposit > 0) {
                    var is_rb = jQuery('.is_roomboss input[type="checkbox"]').val() === 'yes';
                    var depositDueDateFmt = is_rb ? 'Due on Booking' : 'Due Now';
                    html += `<div class="rb-bsummary-pay-line">
                        <div>
                            <span class="rb-bsummary-pay-label">Deposit</span>
                            <span class="rb-bsummary-pay-due">${depositDueDateFmt}</span>
                        </div>
    ...
    const EventHandlers = {
        ...
        handleRoomBossDepositVisibility: function() {
            const $checkbox = jQuery('.is_roomboss input[type="checkbox"]');
            if ($checkbox.length) {
                const isYes = $checkbox.val() === 'yes';
                jQuery('.rb-total-deposit-bar').toggle(!isYes);
            }
        },

        onRoomBossChange: function(e) {
            const isYes = jQuery(e.currentTarget).val() === 'yes';
            jQuery('.rb-total-deposit-bar').toggle(!isYes);
        },
        ...
        onRemoveItem: function(e) {
            ...
            if (Storage.removeItemByIndex(idx)) {
                ...
                // Re-evaluate deposit bar visibility after cart update
                this.handleRoomBossDepositVisibility();
            }
        },
    ...
    return {
        init: function() {
            ...
            // Add RoomBoss deposit visibility logic
            EventHandlers.handleRoomBossDepositVisibility();
            jQuery(document).off('change.rbRoomBoss').on('change.rbRoomBoss', '.is_roomboss input[type="checkbox"]', EventHandlers.onRoomBossChange.bind(EventHandlers));
