...
const AccommodationFilters = (function() {
    ...
    const CONFIG = {
        ...
        ajax: {
            ...
            // url: '<?php echo admin_url('admin-ajax.php'); ?>'earch.run,
        ...
        defaults: {
            ...
            loadingText: 'Loading Your results. </br> This should only take a few seconds.',
    ...
    const UserMetaSync = {
        ...
        save: function(payload) {
            ...
            if (!window.kv_object || !window.kv_object.ajaxurl) return;

            jQuery.ajax({

                url: window.kv_object.ajaxurl,
                ...
                data: {
                    ...
                    action: 'save_search_preferences',
                ...
                success: function(res) {
                    ...
                    // console.log('User meta sync successful:', res);

                },

                error: function(xhr, status, error) {

                    // console.error('User meta sync error:', error);
    ...
    const State = {
        ...
        currentBookingOffset: 0,

        nextBookingOffset: null,
    ...
    const UI = {
        ...
        ensureLoadMoreWrap: function() {

            if (jQuery(CONFIG.selectors.loadMoreWrap).length) {
            ...
            const $results = jQuery(CONFIG.selectors.searchResults);
            ...
            State.cache.$loadMore = jQuery(CONFIG.selectors.loadMore);

        },



        removeLoadMoreWrap: function() {

            jQuery(CONFIG.selectors.loadMoreWrap).remove();

            State.cache.$loadMore = jQuery(CONFIG.selectors.loadMore);

        },

        attachFormHtmlAfterLoadMore: function(formHtml) {
            if (!formHtml) {
    ...
    const FilterData = {
        ...
        collect: function(context) {
            ...
            return {
                ...
                resort: (function() { const v = $scope.find(CONFIG.selectors.resort).val() || ''; return v === 'all' ? '' : v; })(),
        ...
        saveDatesTolocalStorage: function(checkin, checkout) {
            ...
            if (checkin && checkout) {
                ...
                UserMetaSync.save({
                    ...
                    adults: parseInt(localStorage.getItem(CONFIG.storage.adults), 10) || 2,

                    children: parseInt(localStorage.getItem(CONFIG.storage.children), 10) || 0,

                    infants: parseInt(localStorage.getItem(CONFIG.storage.infants), 10) || 0
    ...
    const SearchState = {
            ...
            save: function(context, values = null) {
                ...
                const adults = parseInt(guestState.adults, 10) || 2;
                ...
                localStorage.setItem(CONFIG.storage.adults, String(adults));

                localStorage.setItem(CONFIG.storage.children, String(children));

                localStorage.setItem(CONFIG.storage.infants, String(infants));



                UserMetaSync.save({

                    checkin: checkin || localStorage.getItem(CONFIG.storage.checkin) || '',

                    checkout: checkout || localStorage.getItem(CONFIG.storage.checkout) || '',

                    resort: resort || localStorage.getItem(CONFIG.storage.resort) || '',
    ...
    const Search = {
        ...
        run: function(page = 1, append = false, context = null, bookingOffset = null) {
            ...
            bookingOffset = parseInt(bookingOffset, 10);

            if (isNaN(bookingOffset) || bookingOffset < 0) {
            ...
            if (!append && page === 1) {
                ...
                State.currentBookingOffset = bookingOffset;

                State.nextBookingOffset = null;

                State.maxBookingOffset = null;
            ...
            const ajaxData = {
                ...
                hotel_search: isHotelSearch ? 1 : 0,
            ...
            jQuery.ajax({
                ...
                beforeSend: function() {
                    ...
                    jQuery('#accom-search-results').show();
                ...
                complete: function() {
                    ...
                    const pending = State.pendingBookingOffsetRequest;
                    ...
                    if( jQuery('#accom-search-form').length > 0 && localStorage.red_list !== undefined ) {
                        ...
                        localStorage.removeItem('red_list');
                    ...
                    if (pending && pending.bookingOffset !== null && pending.bookingOffset !== undefined) {
        ...
        handleSuccess: function(data, append, page, context = null) {
            ...
            const shouldShowForm = !!data.form_html;
        ...
        syncBookingPagination: function(data) {

            const offset = parseInt(data.booking_offset, 10);

            const nextOffset = parseInt(data.booking_next_offset, 10);

            const maxOffset = parseInt(data.booking_max_offset, 10);



            if (!isNaN(offset)) {

                State.currentBookingOffset = offset;

            }



            State.nextBookingOffset = !isNaN(nextOffset) ? nextOffset : null;

            State.maxBookingOffset = !isNaN(maxOffset) ? maxOffset : null;

        },



        queueNextBookingOffset: function(data, append, context = null) {
            ...
            if (data.booking_has_more && !isNaN(nextOffset) && nextOffset > State.currentBookingOffset) {
                ...
                State.pendingBookingOffsetRequest = {
    ...
    const Filters = {
        ...
        upsertSelected: function(type, value, label, single = false) {
            ...
            // Use a more robust check for existing filters (matches type AND (value OR label))

            // This handles mismatches between PHP-rendered tabs (e.g. area-echoland) and JS-added tabs (e.g. echoland)

            let filterEl = container.find('.filter-tab').filter(function() {

                const $t = jQuery(this);

                const tType = $t.data('type') || $t.attr('data-type');

                const tVal = $t.data('value') || $t.attr('data-value');
                ...
                return String(tVal) === String(value) || tText === label;

            }).closest('.filter');



            // If duplicates found, prioritize keeping the locked one and remove the unlocked ones

            if (filterEl.length > 1) {

                const locked = filterEl.filter('.locked');

                if (locked.length > 0) {

                    filterEl.not('.locked').remove();

                    filterEl = locked.first();

                } else {

                    filterEl.slice(1).remove();

                    filterEl = filterEl.eq(0);
            ...
            // Ensure correct ordering: Resort first, Reset last, others in middle

            const resetTab = container.find('.filter-tab.reset').closest('.filter');

            if (type === 'resort') {

                container.prepend(filterEl);

            } else if (resetTab.length) {
            ...
            const urlContext = AccommodationFilters.getURLContext();

            if (urlContext) {

                const normalizedUrlType = urlContext.replace(/-/g, '_');

                if (type === urlContext || type === normalizedUrlType || value === urlContext || value === `area-${urlContext}`) {
        ...
        removeSelected: function(type, value = null) {
            ...
            const escapedValue = (value !== null) ? String(value).replace(/"/g, '\\"') : null;



            const selector = value !== null

                ? `.filter-tab[data-type="${escapedType}"][data-value="${escapedValue}"]`

                : `.filter-tab[data-type="${escapedType}"]`;



            container.find(selector).closest('.filter:not(.locked)').remove();
        ...
        lockFilterIfScoped: function(type, value = null) {
            ...
            let selector = `.filter-tab[data-type="${escapedType}"]`;

            if (value !== null) {

                const escapedValue = String(value).replace(/"/g, '\\"');

                selector += `[data-value="${escapedValue}"], [data-value="area-${escapedValue}"]`;

            }

            const filterTab = container.find(selector);
            ...
            filterTab.each(function() {
                ...
                const filterWrapper = jQuery(this).closest('.filter');
            ...
            jQuery.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.success && res.data) {
                        ...
                        // ✅ Handle pre-selection for dynamically loaded areas

                        // First, check for special pathname/area combinations
                        ...
                        // Define special pathname/area combinations.

                        // Use array when one URL should pre-select multiple areas.
                        ...
                        const specialCombinations = {
                            ...
                            // When URL contains /hirafu/, pre-select these 3 areas

                            'hirafu': ['hirafu_village', 'kabayama', 'izumikyo'],
                        ...
                        // If "hirafu" exists anywhere in the URL, apply Hirafu multi-area selection.

                        // Example: /niseko/accommodation/hirafu/

                        if (segments.includes('hirafu')) {

                            checkValues = specialCombinations.hirafu;

                        } else if (specialCombinations.hasOwnProperty(postSlug)) {

                            checkValues = specialCombinations[postSlug];

                        } else {

                            // Check if postSlug is one of the mapped values

                            Object.entries(specialCombinations).some(([key, values]) => {

                                if (values.includes(postSlug)) {
                        ...
                        // Fall back to URL context if no special combination found
                        ...
                        checkValues.forEach(function(checkValue) {
                            ...
                            const normalizedUnderscore = checkValue.replace(/-/g, '_');

                            const normalizedHyphen = checkValue.replace(/_/g, '-');



                            const $targetInput = $container.find(

                                `#area-${checkValue}, ` +

                                `#area-${normalizedUnderscore}, ` +

                                `#area-${normalizedHyphen}, ` +

                                `input[value="${checkValue}"], ` +

                                `input[value="${normalizedUnderscore}"], ` +

                                `input[value="${normalizedHyphen}"]`
                            ...
                            if ($targetInput.length > 0) {
                                ...
                                const actualValue = $targetInput.val();



                                $targetInput.prop('checked', true).trigger('change');

                                $targetInput.siblings('label').css('pointer-events', 'none');



                                Filters.lockFilterIfScoped('area', actualValue);
    ...
    const Events = {
        ...
        init: function() {
            ...
            // Load more (delegated so it still works after the wrapper is reinserted)

            jQuery(document).off('click', CONFIG.selectors.loadMore).on('click', CONFIG.selectors.loadMore, this.onLoadMore.bind(this));
        ...
        onLoadMore: function() {
            ...
            const nextBookingOffset = parseInt(State.cache.$loadMore.attr('data-booking-next-offset'), 10);
        ...
        onRemoveFilterTab: function(e) {
            ...
            if (type === 'price') {
                ...
                jQuery(`input[name="${type}[]"][value="${value}"], input[name="${type}"][value="${value}"]`).prop('checked', false);
        ...
        onResortChange: function(e) {
            ...
            // Sync all resort inputs (Selects and Radios) across the page
            ...
            // Use the main search form as context to ensure global state is preserved

            const $context = jQuery(CONFIG.selectors.searchForm).first();

            SearchState.save($context);



            const resortKey = resortSlug === 'all' ? '' : resortSlug.replace(/-accommodation$/, '').replace(/ accommodation$/, '');
        ...
        onFilterInputChange: function(e) {
            ...
            const nameAttr = ($input.attr('name') || '').replace('[]', '');

            

            // Use consistent type identification: group name for checkboxes, data-type/name for radios

            const type = isRadio ? ($input.data('type') || nameAttr) : nameAttr;
                    ...
                    onSelect: function ({date, formattedDate}) {
                        ...
                        jQuery(this).dateDropper({
                            ...
                            onChange: function(res) {
                                ...
                                jQuery(CONFIG.selectors.checkin).val(actualSelectedDate);
                                    ...
                                    if (instance) {
                                        ...
                                        new AirDatepicker(this, {
                                            ...
                                            onSelect: function ({formattedDate: dateStrOut}) {
                                                ...
                                                jQuery(CONFIG.selectors.checkout).val(dateStrOut);
            ...
            $checkout.each(function() {
                ...
                new AirDatepicker(this, {
                    ...
                    onSelect: function ({formattedDate}) {
                        ...
                        jQuery(CONFIG.selectors.checkout).val(formattedDate);
                ...
                jQuery(this).dateDropper({
                    ...
                    onChange: function(res) {
                        ...
                        const maxDate = res.date.m + '/' + res.date.d + '/' + res.date.Y;
    ...
    return {
        ...
        init: function() {
            ...
            jQuery(document).on('click', '.sb-submit', function() {
                ...
                var resortVal = jQuery(this).parents('.search-row').find('.js-sb-resort').val() || '';

                if (resortVal && resortVal !== 'all') {

                    var redirectPath = '/'+resortVal.replace('-', '/')+'/';

                    localStorage.setItem('list_red', redirectPath);
            ...
            if( localStorage.getItem('list_red') !== null ){

                var list_redirect = localStorage.getItem('list_red');

                if (list_redirect && list_redirect === pathname) {

                    localStorage.removeItem('list_red');
        ...
        exposeGlobals: function() {
            ...
            window.doSearch = (el) => {
                ...
                const resortVal = (jQuery(el).closest('.search-row').find(CONFIG.selectors.resort).val()
                                ...
                                || jQuery(CONFIG.selectors.resort).first().val() || '');
                ...
                var single_form = jQuery('#room-filter-form');

                jQuery( '.mobFilterModal' ).length > 0 && jQuery( '.mobFilterModal' ).hasClass('open') ? jQuery( '.mobFilterModal' ).removeClass('open') : '';
            ...
            window.resetFilters = (el) => {
                ...
                if (pathname.indexOf('/accommodation/') !== -1) {
        ...
        restrictBookingGuests: function() {
            ...
            const lsAdults = parseInt(getStoredValue(CONFIG.storage.adults), 10) || 0;

            const lsChildren = parseInt(getStoredValue(CONFIG.storage.children), 10) || 0;
            ...
            const restrictOptions = function(selector, limit, forcedValue = null) {
                ...
                jQuery(selector).each(function() {
                    ...
                    $select.find('option').each(function() {
                        ...
                        const parsedValue = parseInt(optionValue, 10);



                        if (optionValue !== '' && !isNaN(parsedValue) && parsedValue > limit) {
            ...
            // restrictOptions('.rb-adults', adultLimit, adultLimit);

            // restrictOptions('.rb-children', childInfantLimit);

            // restrictOptions('.rb-infants', childInfantLimit);

        },



        checkSpecialAreaCombinations: function() {

            // Get the current pathname (post slug)
            ...
            const specialCombinations = {
                ...
                'niseko-village-higashiyama': 'niseko-village',

                'happo-one-village': 'happo',

                'wadano_no_mori': 'wadano'

            };



            // Check if current post slug has a special area combination

            if (specialCombinations.hasOwnProperty(postSlug)) {

                const areaValue = specialCombinations[postSlug];

                

                // Find and check the corresponding area checkbox

                const $areaCheckbox = jQuery(

                    `input[name="area[]"][value="${areaValue}"], ` +
                ...
                if ($areaCheckbox.length > 0) {

                    $areaCheckbox.prop('checked', true).trigger('change');
        ...
        initializeFromInputs: function() {
            ...
            // 0. Handle special pathname/area combinations

            this.checkSpecialAreaCombinations();
            ...
            if (urlContext) {
                ...
                // Updated selector to include #area- prefix common in base areas

                const $targetInput = jQuery(

                    `#${normalizedId}, #${urlContext}, #area-${urlContext}, #area-${normalizedId}, ` +

                    `input[value="${urlContext}"], input[value="${urlContext}-accommodation"], input[name="${normalizedId}"]`
                ...
                if ($targetInput.length > 0) {
                    ...
                    const type = ($targetInput.data('type') || $targetInput.attr('name') || urlContext).replace('[]', '');

                    const value = $targetInput.val();

                    Filters.lockFilterIfScoped(type, value);
            ...
            // 4. Update Resort-based Areas - WAIT for this to complete before searching

            const resortSlug = jQuery(CONFIG.selectors.resort).first().val()?.toLowerCase() || '';

            const resortKey = resortSlug === 'all' ? '' : resortSlug.replace('-accommodation', '');

            

            // Wait for updateBaseArea to complete, then trigger search
            ...
            Filters.updateBaseArea(resortKey).then(() => {
                ...
                if (isAccommodationPage || (checkin && checkout)) {
                    ...
                    console.log( 'Initial search triggered by initializeFromInputs' );
        ...
        search: function(page = 1, append = false) {
            ...
            console.log( 'AccommodationFilters.search called with page:', page, 'append:', append );
